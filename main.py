...
# Importar la biblioteca seaborn para gráficos más atractivos
import seaborn as sns

# Crear un DataFrame con las predicciones y las observaciones reales
df_prediccion = pd.DataFrame({'Predicciones': predicciones_stats, 'Observaciones': y_test})

# Graficar las predicciones vs. observaciones reales
sns.lmplot(x='Observaciones', y='Predicciones', data=df_prediccion, height=6, aspect=1.5)
plt.title('Predicciones vs. Observaciones Reales')
plt.xlabel('Relación calidad-precio real')
plt.ylabel('Relación calidad-precio predicha')
plt.show()

# Calcular los residuos
residuos = y_test - predicciones_stats

# Graficar un gráfico de dispersión de los residuos
plt.scatter(predicciones_stats, residuos)
...
# Importar bibliotecas para gráficos Q-Q
...
# Eliminar 'const' y 'Limpieza' del DataFrame X
X_nuevo = X.drop(['const', 'Limpieza'], axis=1)
